export function countConverter(count: any) {
    let result;
    if (count < 1000) {
        result = count;
    } else if (count < 10000) {
...
export function agoConverter(dateString: string) {
...
export function dateConverter(publishedAt: any) {
...
export function tagMaker(tags: any) {
    let result = '';
    let i = 0;
    while (i < 3) {
        result += `#${tags[i]} `;
